...
`EngineState::coordinate_replacement` 还协调新对局和揭棋文档打开。候选对局先在锁外完成构造或文件重放，生命周期锁内先复核 `SessionToken`，再停止事件转发与外部进程，最后通过 `GameState::replace` 再次复核 token 并原子换局。锁序固定为生命周期锁 → 引擎任务/会话锁；文件 I/O 期间不持有 `GameState` 或引擎锁。
...
外部引擎和开局库只支持普通象棋。活动会话为揭棋时，`SessionCapabilities.analyze` 与 `query_book` 禁用；引擎分析 IPC 也在后端复核能力，不能靠前端隐藏控件绕过。切换到揭棋会停止现有搜索、子进程和转发任务；之后切回普通象棋不会自动恢复已取消的旧分析请求。
