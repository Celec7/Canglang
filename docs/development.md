...
- **GitHub 网页一键发版**：在 GitHub Actions 的 `release` 工作流选择升级级别（或输入指定版本号）直接触发。工作流自动更新版本、打标签、提取 Release Notes，在平台构建前创建一个 Draft Release，所有矩阵任务复用该 Release 上传产物；全部成功后按草稿选项发布。
