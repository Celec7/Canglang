...
`ManualService` 按路径扩展名分派 `.pgn` 与 `.xqf`。空路径、文件不存在、扩展名不支持、IO 失败与解析失败分别映射到 `ManualError`。当前导出支持 `.pgn` 和独立的 `.xqf` canonical v10 编码路径，不能改名把任意格式当 PGN 写入；XQF 路径不复用 PGN 文本写出。
