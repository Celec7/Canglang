...
async function first() {
  stopPlayback();
  if (game.capabilities.jump.enabled) await game.jumpTo(0);
}
...
async function last() {
  stopPlayback();
  if (game.capabilities.jump.enabled) await game.jumpTo(game.history.length);
}
</script>
...
<template>
  <div class="flex items-center justify-between gap-1 border-t bg-card/60 px-3 py-2 text-xs">
    ...
    <div class="flex items-center gap-0.5" aria-label="复盘导航">
      <Button variant="ghost" size="icon" class="size-6" :disabled="!game.capabilities.jump.enabled || game.currentPly === 0" aria-label="起始局面 (Home)" title="起始局面 (Home / Shift+[)" @click="first">
      ...
      <Button variant="ghost" size="icon" class="size-6" :disabled="!game.capabilities.jump.enabled || game.currentPly === game.history.length" aria-label="最新局面 (End)" title="最新局面 (End / Shift+])" @click="last">
